...
resh_refs AS (
    SELECT r.original_word, r.new_word,
           MIN(b1.title || ' ' || v1.chapter || ':' || v1.verse) as first_resh_verse,
           l1.def as def_resh
    FROM replacements r
    ...
    WHERE b1.ord != 0  -- Exclude Book 0
    GROUP BY r.original_word, r.new_word, l1.def
),
tav_refs AS (
    SELECT r.original_word, r.new_word,
           MIN(b2.title || ' ' || v2.chapter || ':' || v2.verse) as first_tav_verse,
           l2.def as def_tav
    FROM replacements r
...
SELECT 
    ...
    rr.def_resh,
    tr.def_tav,
    rr.first_resh_verse as first_verse_with_resh,
    tr.first_tav_verse as first_verse_with_tav
FROM resh_refs rr
